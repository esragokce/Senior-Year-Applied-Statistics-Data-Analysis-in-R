...
Twoing (İkileme), karar ağaçlarında kullanılan bir bölünme kriteridir. Gini ve Entropy’den farklı olarak, 
bir düğüm içindeki saflığı değil, bölünme sonucu oluşan iki alt düğümün birbirinden ne kadar farklı olduğunu ölçer. 
Yani Twoing, alt düğümlerin sınıf dağılımlarını mümkün olduğunca birbirinden uzaklaştıracak şekilde bölünme yapar.
Bu özelliği sayesinde özellikle çok sınıflı problemlerde avantajlıdır, çünkü sınıfları iki gruba ayırarak her bölünmede 
en iyi ayrımı bulmaya çalışır. Twoing formülü, sol ve sağ düğümlerin büyüklüklerini ve sınıf dağılımları arasındaki 
farkları dikkate alarak hesaplanır. Pratikte Gini kadar yaygın kullanılmasa da, özellikle sınıfların dengeli 
dağılmadığı veya çok sayıda sınıf bulunan veri setlerinde başarılı sonuçlar verir. 
R’da rpart paketinde parms = list(split = "twoing") parametresi ile kullanılabilir.
...
ToothGrowth, R içinde hazır olarak bulunan ve C vitamini takviyesinin diş büyümesine etkisini inceleyen bir veri setidir. 
Bu veri seti, domuz yavrularında farklı C vitamini dozlarının ve kaynaklarının diş uzunluğu üzerindekietkisini 
ölçmek için yapılan bir deneyin sonuçlarını içerir.
...
Grafikte tüm cp değerlerinde ROC (AUC) değeri yaklaşık 0.78 civarında sabit görünüyor.
Bu, modelin cp değerinden etkilenmediğini gösterir.
...
klasik mtcars veri seti kullanılarak bir aracın özelliklerine (beygir gücü, ağırlık, yakıt tüketimi vb.) 
bakarak şanzıman tipinin (Otomatik/Manuel) tahmin edilmesi amaçlanmıştır.
